...
<section class="container py-5">
    ...
    @endif
    <form class="container d-flex flex-column w-75" action="{{route('client.update', $client->id)}}" enctype="multipart/form-data" method="post">
        @csrf
        @method('PUT')
        ...
        <input type="text" value="{{$client->firstname}}" name="firstname" id="firstname">

        <label for="comment">Commentaire : </label>
        <textarea name="comment" id="comment" cols="30" rows="4">{{$client->comment}}</textarea>
        {{-- @php
            $a=0;
        @endphp --}}
